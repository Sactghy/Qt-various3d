...
MainWindow::MainWindow( MainWindow *ptTh, QWidget *parent ) : QMainWindow(parent), ui(new Ui::MainWindow), pntThis(ptTh)
{
   ...
   for ( int x = 0; x <= 494; x++ ) { for ( int y = 0; y <= 338; y++ ) {
    ...
    control = new Dialog{pntThis}; info = new InfoDial{pntThis}; back = new Back{pntThis};
...
{
    delete ui; delete control; delete info; delete back;
...
void MainWindow::startF()
{
      ...
      for ( int x = 0; x <=780; x++ ) { int p = ( ( 780 * y ) + x ) * 4; bool cursor {false};
       ...
       double is1opq{};
         ...
         struct it_BVols {
           ...
           // std::vector<BSD> internalBVols {};

            double dist1{INFINITY}, dist2{INFINITY}, prevLen{};
    ...
    while ( yL0 < 780 ) {
     ...
     thGrd thG0 { std::thread ( processLine, yL0+0 ) };
     thGrd thG1 { std::thread ( processLine, yL0+1 ) };
     thGrd thG2 { std::thread ( processLine, yL0+2 ) };
     thGrd thG3 { std::thread ( processLine, yL0+3 ) };
     thGrd thG4 { std::thread ( processLine, yL0+4 ) };
     thGrd thG5 { std::thread ( processLine, yL0+5 ) };
     thGrd thG6 { std::thread ( processLine, yL0+6 ) };
     thGrd thG7 { std::thread ( processLine, yL0+7 ) };
     thGrd thG8 { std::thread ( processLine, yL0+8 ) };
     thGrd thG9 { std::thread ( processLine, yL0+9 ) };
